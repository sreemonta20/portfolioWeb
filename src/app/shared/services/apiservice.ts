...
export class ApiService {
  ...
  getSingle(getByIdUrl: string): Observable<any> {
    const getUrlfinal = this.baseURL + getByIdUrl;
    const headers = new HttpHeaders({
      'content-type': 'application/json',
      Authorization: 'Bearer ' + JSON.parse(localStorage.getItem('token') || '{}')
    });

    return this.http.get(getUrlfinal, { 'headers': headers });
  }
  // Get All
  ...
  methodPost(apiRout: string, model: any): Observable<any> {
    const apiRoutfinal1 = apiRout;
    const headers = { 'Content-Type': 'application/json' };
